...
@Injectable()
export class BackgroundImageHandler extends BasePipelineHandler {
  protected readonly doneEvent = PipelineEvent.BG_IMG_DONE;
  protected readonly stepKey = StepKey.GENERATE_BACKGROUND_IMAGES;
  ...
  @OnEvent(PipelineEvent.BG_IMG_START)
  ...
  protected execute(episodeId: string) {
    return this.imageService.generateBackgroundImages(episodeId);
  }
}
